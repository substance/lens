...
Collection.Prototype = function() {
  ...
  this.determineItems = function() {
    ...
    _.each(items, function(item) {
      // HACK: absusing the component API
      // If node is not in the container at all, getComponentsForNode
      // returns undefined
      var components = _.map(container.getComponent([item.id]));
      var comp = _.first(components);
      var pos;
      var isShown;
      if (comp) {
        isShown = true;
